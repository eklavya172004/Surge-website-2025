...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen text-gray-800 bg-white">
      ...
      >
        {/* Header */}
<motion.div className="text-center mb-16 px-4 sm:px-6" variants={itemVariants}>
  <motion.div
    className="inline-flex items-center gap-3 mb-6"
    whileHover={{ scale: 1.05 }}
    transition={{ type: "spring", damping: 15 }}
  >
    <div className="p-3 sm:p-4 bg-gradient-to-r from-blue-500 to-purple-600 rounded-2xl">
...
<div className="max-w-full sm:max-w-4xl mx-auto space-y-8 px-4 sm:px-0">

          {/* Event Selection Card */}
<motion.div
  className="relative bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 shadow-xl border border-gray-200 hover:shadow-2xl transition-all duration-300 overflow-visible"
>
  <motion.select
    className="w-full p-3 sm:p-4 rounded-xl sm:rounded-2xl border-2 border-gray-300 text-gray-800 text-base sm:text-lg bg-white
...
<AnimatePresence>
  {selectedEventData && (
    <motion.div 
      className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 shadow-xl border border-gray-100"
    ...
    >
      <motion.h2 
        className="text-2xl sm:text-3xl font-bold text-gray-800 mb-6 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4"
        initial={{ x: -20 }}
        animate={{ x: 0 }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8 mb-8">
        ...
        >
          {selectedEventData.venue && (
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 text-gray-700 p-2 sm:p-3 bg-blue-50 rounded-lg sm:rounded-xl">
          ...
          {(selectedEventData.dateFrom && selectedEventData.dateTo) && (
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 text-gray-700 p-2 sm:p-3 bg-green-50 rounded-lg sm:rounded-xl">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 text-gray-700 p-2 sm:p-3 bg-purple-50 rounded-lg sm:rounded-xl">
          ...
          {selectedEventData.pricePerPlayer && (
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-2 sm:p-3 bg-gradient-to-r from-green-100 to-green-200 rounded-lg sm:rounded-xl">
      ...
      {selectedEventData.rules && (
        <motion.div 
          className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border-2 border-blue-200 mb-8"
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.5 }}
        >
          <h3 className="font-bold text-blue-700 mb-4 text-base sm:text-lg">Tournament Rules & Guidelines</h3>
      ...
      <motion.div 
        className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border-2 border-indigo-200 mb-8"
        initial={{ y: 20, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ delay: 0.6 }}
      >
        <h3 className="font-bold text-indigo-700 mb-4 text-base sm:text-lg flex items-center gap-2">
  ...
  {selectedEventData && (
    ...
    >
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4 sm:gap-0">
        <motion.h3 
          className="text-xl sm:text-2xl font-bold text-gray-800 flex items-center gap-3 sm:gap-4"
          initial={{ x: -20 }}
          animate={{ x: 0 }}
        >
        ...
        <motion.div 
          className="bg-gradient-to-r from-indigo-100 to-blue-100 px-4 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl border-2 border-indigo-200 text-center sm:text-left"
          whileHover={{ scale: 1.05 }}
        >
          ...
          {playerDetails.map((player, i) => (
            ...
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-2 sm:gap-0">
                <motion.h4 
                  className="text-base sm:text-lg font-bold text-gray-800 flex items-center gap-2 sm:gap-3"
                  whileHover={{ scale: 1.05 }}
                >
                  Player {i + 1}
                </motion.h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <motion.div className="space-y-1 sm:space-y-2" whileFocus={{ scale: 1.02 }}>
                  <label className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm font-semibold text-gray-700 mb-1 sm:mb-2">
                    <User className="w-3 h-3 sm:w-4 sm:h-4 text-blue-500" />
                    Full Name
                  </label>
                  <input
                    type="text"
                    className="w-full p-3 sm:p-4 rounded-xl border-2 border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-300 text-gray-800 bg-white shadow-sm text-sm sm:text-base"
...
{playerDetails.length < (selectedEventData.maxPlayers ?? 100) && (
  <motion.button
    onClick={addPlayer}
    className="w-full flex items-center justify-center gap-2 sm:gap-3 p-4 sm:p-6 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white font-bold text-base sm:text-lg rounded-xl sm:rounded-2xl shadow-lg mb-8 group"
...
{/* Action Buttons */}
<div className="pt-6 border-t-2 border-gray-100 flex flex-col sm:flex-row gap-3 sm:gap-4">
  <motion.button
    onClick={handleAddToCart}
    disabled={!selectedEvent || isSubmitting}
    className="flex-1 p-4 sm:p-6 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed text-white font-bold text-base sm:text-xl rounded-xl sm:rounded-2xl shadow-xl flex items-center justify-center gap-3 sm:gap-4"
  ...
  <Link href="/dashboard/cart">
    <motion.button 
      className="p-4 sm:p-6 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-bold text-base sm:text-xl rounded-xl sm:rounded-2xl shadow-xl flex items-center justify-center gap-2 sm:gap-3"
      whileHover={{ scale: 1.02, y: -2 }}
      whileTap={{ scale: 0.98 }}
    >
...
{playerDetails.length > 0 && selectedEventData.minPlayers && playerDetails.length < selectedEventData.minPlayers && (
  <motion.p 
    className="text-amber-600 text-center mt-4 flex items-center justify-center gap-2 bg-amber-50 p-3 rounded-xl text-sm sm:text-base"
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
  >
